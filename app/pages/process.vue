          ...
          <div :key="activeTab" class="bg-white dark:bg-white/5 backdrop-blur-sm rounded-2xl border border-gray-200 dark:border-white/10 p-8 shadow-lg">
            ...
            <div class="mb-8">
              <div class="flex items-start justify-between mb-4">
                <h2 class="text-2xl md:text-3xl font-light text-gray-900 dark:text-white">
                  {{ sections[activeTab]?.title }}
                </h2>
                <span class="text-xs px-3 py-1 bg-secondary/10 dark:bg-secondary-dark/10 text-secondary dark:text-secondary-dark rounded-full font-light">
              ...
              <div
                v-if="activeTab === 0"
                class="inline-flex items-center gap-2 px-4 py-2 bg-secondary/10 dark:bg-secondary-dark/20 border border-secondary/30 dark:border-secondary-dark/40 rounded-lg mb-6">
                <Icon name="lucide:trophy" class="w-5 h-5 text-secondary dark:text-secondary-dark" />
              ...
              <div v-if="sections[activeTab]?.features" class="grid md:grid-cols-2 gap-6 mb-6">
                <div v-for="(feature, idx) in sections[activeTab]?.features" :key="idx">
                  <h4 class="font-light text-gray-900 dark:text-white mb-3 flex items-center gap-2">
                  ...
                  <ul class="space-y-2 text-gray-600 dark:text-gray-300 text-sm">
                    <li v-for="(item, i) in feature.items" :key="i" class="flex items-center gap-2">
                      <span class="w-1.5 h-1.5 bg-secondary dark:bg-secondary-dark rounded-full"></span>
            ...
            <!-- Diagram & Code Toggle -->
            <div class="mb-6 flex justify-end" v-if="sections[activeTab]?.code">
            ...
            <!-- Diagram View -->
            <div v-show="!showCode" class="mermaid-container">
              <div class="mermaid bg-gray-50 dark:bg-[#0B1120] p-8 rounded-xl border border-gray-100 dark:border-white/5 overflow-x-auto" v-html="currentSvg">
              </div>
            </div>

            <!-- Code View -->
            <div v-if="showCode && sections[activeTab]?.code" class="relative group">
              <div class="absolute -top-3 right-4 px-2 py-1 bg-gray-800 text-gray-300 text-xs rounded-md font-mono border border-gray-700">
                {{ sections[activeTab]?.code?.filename }}
              </div>
              <pre class="bg-[#0B1120] text-gray-300 p-6 rounded-xl border border-gray-700 overflow-x-auto text-sm font-mono leading-relaxed"><code>{{ sections[activeTab]?.code?.content }}</code></pre>
            </div>

            <!-- Navigation Arrows -->
            <div class="flex justify-between mt-8 pt-6 border-t border-gray-200 dark:border-white/10">
              <button
                @click="previousTab"
...
const nextTab = () => {
  if (activeTab.value < sections.value.length - 1) activeTab.value++;
};
...
// Re-render Mermaid when tab changes
